...
def fix_unk_from_text_with_tokenizer_p(span, text, tokenizer, unk='<unk>'):
    # unk_id = tokenizer.get_vocab()[unk]
    unk_id = tokenizer.vocab.to_indices(unk)
    # tokenized_span = tokenizer.encode_plus(span,
    ...
    if tokenized_span[0] == unk_id and matched[0][0] > 0:
        ...
        # pre_strip = tokenizer.convert_ids_to_tokens(previous_token)
    else:
        previous_token = []
        pre_strip = ""
    ...
    if tokenized_span[-1] == unk_id and matched[0][1] < len(tokenized_text) - 1:
        ...
        # next_strip = tokenizer.convert_ids_to_tokens(next_token)
    else:
        next_token = []
        next_strip = ""
...
def fix_unk_from_text_with_tokenizer(span, text, tokenizer, unk='<unk>'):
    ...
    if tokenized_span[0] == unk_id and matched[0][0] > 0:
        previous_token = [tokenized_text[matched[0][0] - 1]]
        # pre_strip = tokenizer.vocab.to_tokens(previous_token[0])
        pre_strip = tokenizer.convert_ids_to_tokens(previous_token)[0]
    else:
        previous_token = []
        pre_strip = ""
    ...
    if tokenized_span[-1] == unk_id and matched[0][1] < len(tokenized_text) - 1:
        next_token = [tokenized_text[matched[0][1] + 1]]
        # next_strip = tokenizer.vocab.to_tokens(next_token[0])
        next_strip = tokenizer.convert_ids_to_tokens(next_token)[0]
    else:
        next_token = []
        next_strip = ""
