...
def filter_words(w_map, emb_array, ck_filenames):
    ...
    for filename in ck_filenames:
        for line in open(filename, 'r'):
            if not (line.isspace() or (len(line) > 10 and line[0:10] == '-DOCSTART-')):
                ...
                word = line[0]
                vocab.add(word) #变行前的word
                word = word.lower()
                vocab.add(word) #变形后的word
    new_w_map = {}
    new_emb_array = []
    for (word, idx) in w_map.items():
        if word in vocab or word in ['<unk>', '<s>', '< >', '<\n>']:
            ...
            new_emb_array.append(emb_array[idx]) #添加特殊的字符
...
def read_noisy_corpus(lines):
    ...
    for line in lines:
        if not (line.isspace() or (len(line) > 10 and line[0:10] == '-DOCSTART-')):
            ...
            # 3. Safe or dangerous?   <-- this is optional
            token = line[0] #词
            chunk_boundary = line[1] #边界 
            entity_types = line[2] #类型
            ...
            tmp_fl.append(token) #词
            tmp_lcml.append(safe) #mask值
            if safe:
                tmp_lcl.append(chunk_boundary) #tie or break的类型
                if 'I' == chunk_boundary:
                    type_list = entity_types.split(',')
                    tmp_lpl.append(1)#tie or break的数字类型
                    tmp_ltl.append(type_list) #类型
                else:
                    tmp_lpl.append(0)
        elif len(tmp_fl) > 0:
...
def encode_folder(input_folder, output_folder, w_map, c_map, cl_map, tl_map, c_threshold = -1):
    ...
    # Token,tie or break,saft or dangerous,tie or break的数字类型,Type
    if c_threshold > 0:
        c_count = dict() #统计每个字符
        ...
        for key in c_set:
            if key not in c_map:
                c_map[key] = len(c_map) #分散成每个字符建立词典跟索引
    ...
    for f_l, l_c, l_c_m, l_m, l_t in zip(features, labels_chunk, labels_chunk_mask, labels_point, labels_typing):
        ...
        for i_f, i_m in zip(f_l[1:-1], l_c_m[1:-1]):
            tmp_w = tmp_w + [w_map.get(i_f, w_map.get(i_f.lower(), w_unk))] * len(i_f) + [w_con] #词的位置全填充跟字对齐,con为分隔符作用
            tmp_c = tmp_c + [c_map.get(t, c_unk) for t in i_f] + [c_con] #字的位置添加上<s>跟< >
            tmp_mc = tmp_mc + [0] * len(i_f) + [i_m] #词加上前后两个值与mask的值
        ...
        tmp_lc = [cl_map[tup] for tup in l_c[1:]] #tie or break数字类型
        tmp_mt = l_m[1:] #tie or break数字类型 
        tmp_lt = list()
        for tup_list in l_t:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_train', default='./models/BC5CDR/annotations.ck')
    parser.add_argument('--input_testa', default="./data/BC5CDR/truth_dev.ck")
    parser.add_argument('--input_testb', default="./data/BC5CDR/truth_test.ck")
    parser.add_argument('--pre_word_emb', default="./data/glove.100.pk")
    parser.add_argument('--output_folder', default="./encoded_data/")
    args = parser.parse_args()

    with open(args.pre_word_emb, 'rb') as f:
        w_emb = pickle.load(f)
        w_map = w_emb['w_map'] #词
        emb_array = w_emb['emb_array'] #向量
    ...
    #           'AspectTerm': 1}
    tl_map = build_label_mapping(args.input_train, args.input_testa, args.input_testb) #构建entity类型
    cl_map = {'I': 0, 'O': 1}
