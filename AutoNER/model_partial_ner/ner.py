...
class NER(nn.Module):
    ...
    def __init__(self, rnn, 
        ...
        self.to_chunk = highway(self.rnn_outdim) #trans跟gate(300,300)的输出
        self.to_type = highway(self.rnn_outdim)

        if self.add_proj:
            self.to_chunk_proj = nn.Linear(self.rnn_outdim, y_dim) #(300,50)
            self.to_type_proj = nn.Linear(self.rnn_outdim, y_dim)#(300,50)
            self.chunk_weight = nn.Linear(y_dim, 1)#(50,1)
            self.type_weight = nn.Linear(y_dim, y_num)#(50,3)
    ...
    def forward(self, w_in, c_in, mask):
        ...
        mask = mask.unsqueeze(2).expand_as(out).bool()
