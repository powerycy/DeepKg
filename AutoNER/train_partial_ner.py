...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_name', default='autoner')
    parser.add_argument('--git_tracking', action='store_true')

    parser.add_argument('--eval_dataset', default='./encoded_data/test.pk')
    parser.add_argument('--train_dataset', default='./encoded_data/train_0.pk')
    ...
    ner_model.rand_ini() #初始化参数
    ner_model.load_pretrained_word_embedding(torch.FloatTensor(emb_array)) #加载embedding
    ner_config = ner_model.to_params() #参数的值为多少
    ner_model.to(device) 
    
    optim_map = {'Adam' : optim.Adam, 'Adagrad': optim.Adagrad, 'Adadelta': optim.Adadelta, 'SGD': functools.partial(optim.SGD, momentum=0.9)}
    if args.lr > 0:
        optimizer=optim_map[args.update](ner_model.parameters(), lr=args.lr) #更新参数
    else:
    ...
    try:
        ...
        for indexs in range(args.epoch):
            ...
            for word_t, char_t, chunk_mask, chunk_label, type_mask, type_label in train_loader.get_tqdm(device):
                ner_model.zero_grad()
                output = ner_model(word_t, char_t, chunk_mask.bool()) #(4652,300)

                chunk_score = ner_model.chunking(output) #(4652)
                chunk_loss = crit_chunk(chunk_score, chunk_label) #二分类判断tie or break

                type_score = ner_model.typing(output, type_mask.bool()) #判断ner的类型
